...
export default function BookingPage() {
  ...
  async function cacelBooking(e, id) {
    e.preventDefault();
    const userConfirmed = window.confirm(
      "Are you sure you want to delete this place permanently?"
    );
    if (userConfirmed) {
      await axios.post("/cancelbooking", { id: id }).then((response) => {
        ...
        <div className="flex flex-col gap-4">
          {bookings.map((booking) => {
            ...
            return (
              <div
                key={booking._id}
                className="bg-gray-200 p-4 rounded-md flex gap-4 flex-col md:flex-row "
              >
                <img
                  src={`http://localhost:8081/uploads/${booking.place.photos[0]}`}
                  alt={booking.place.title}
                  className="md:w-48 sm:w-full h-40 object-cover rounded-md "
                />
                <div className="flex flex-col justify-between">
                  ...
                  <div className="mt-1 flex gap-4 text-gray-500">
                    ...
                    <p className="">Contact Number: {booking.number}</p>
                  </div>
                  <div className="flex gap-2 mt-2 text-xl font-semibold items-center">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                    ...
                    <span className="font-normal text- text-gray-500">
                      for {nights} nights
                    </span>
                  </div>
                  <div className="mt-2 flex items-center justify-around">
                    <button className="px-5 text-white py-1 rounded-md bg-primary">
                      Pay Now
                    </button>
                    <button
                      onClick={(e) => {
                        return cacelBooking(e, booking._id);
                      }}
                      className="px-5 text-white py-1 rounded-md bg-gray-400"
                    >
                      Cancel
                    </button>
                  </div>
